...
const FormEditData = ({ isOpen, handleCloseForm, shiftData }) => {
  ...
  useEffect(() => {
    if (shiftData) {
      const daysArray = Array.isArray(shiftData.description)
        ? shiftData.description
        : typeof shiftData.description === 'string'
          ? shiftData.description.split(',').map(d => d.trim())
          : []
      setFormData({
  ...
  const handleDayChange = (e) => {
    ...
    const updated = checked
      ...
      : formData.description.filter(day => day !== value)
    ...
    <div className={`${isOpen ? 'flex' : 'hidden'} fixed top-0 left-0 z-50 w-full h-full bg-black/70 backdrop-blur-[3px] items-center justify-center`}>
      <div className="relative w-full max-w-md bg-white rounded-lg shadow-lg p-5">
        ...
        <select
            ...
            required
            className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg block w-full p-2.5 focus:outline-none focus:ring-2 focus:ring-blue-600"
        >
        ...
        {formData.shift === '' && <p className="text-red-500 text-sm">Vui lòng chọn ca làm</p>}
          ...
          <label className="mt-4 block">
            ...
            <input
              ...
              required
              className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg block w-full p-2.5 focus:ring-blue-600 focus:outline-none"
            />
          </label>
          ...
          <label className="mt-4 block">
            ...
            <div className="relative">
              <div
                ...
                className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg px-3 py-2.5 cursor-pointer select-none hover:ring-1 hover:ring-blue-400"
              >
                {formData.description.length > 0
                  ? formData.description.join(', ')
                  : <span className="text-gray-400">-- Chọn ngày làm --</span>}
              </div>
        ...
        <div className="flex items-center justify-end space-x-3 mt-6">
          <BtnSubmit
            onClick={handleCloseForm}
            className="border border-gray-300 py-2.5 px-5 hover:bg-gray-50"
          >
            Hủy
          </BtnSubmit>
          <BtnSubmit
            onClick={handleSubmit}
            className="bg-blue-600 text-white py-2.5 px-5"
          >
            Lưu
          </BtnSubmit>
        </div>
